...
export class MemoryController {
  async createMemory(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const userId = req.headers['x-user-id'] as string || 'mock-user-123';
  ...
  async getMemory(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const { id } = req.params;
  ...
  async generateQR(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const updated = await memoryService.updateMemory(id, { qrCodeUrl: qrLink } as any);
